	  		...
	  		if (req.readyState == '4') {
	  			...
	  			switch (req.status) {
	                ...
	                case 205:
	                    ...
	                    break;
	                default:
	                    callbackError(JSON.parse(req.responseText));
                }
	    	}
		}
...
//		}, callbackError);
	...
	var getProductAndReleaseList = function (callback, callbackError, region) {

//		check(region);

//		sendRequest('GET', 'catalog/productandrelease', undefined, function (resp) {
//			var productList = x2js.xml_str2json(resp);
//			callback(productList.productAndReleaseDtoes.productAndReleaseDto_asArray);
//		}, callbackError);
	};
	...
	var installProductInstance = function (ip, fqn, product, hostname, callback, callbackError, region) {

		check(region);

		var i = {productInstanceDto: {vm: {ip: ip, fqn: fqn, osType: "", hostname: hostname}, product: product}};
...
//			callback(productInstances.productInstanceDtoes.productInstanceDto);
//		}, callbackError);
	};
    ...
    return {
	    ...
	    getProductAndReleaseList: getProductAndReleaseList,
